...
CREATE INDEX_CIRCUITO
ON circuito(id_pais).

-- 4. carrera:

CREATE INDEX_CARRERA
ON carrera(carrera_cant_vueltas); // no me convence este pero bueno, por las dudas 
...
ON telemetria_auto(vehiculo_numero,cod_escuderia,circuito_codigo);

CREATE INDEX INDEX_TELE_AUTO_IV
ON telemetria_auto(cod_sector,cod_carrera);

CREATE INDEX INDEX_TELE_AUTO_V
